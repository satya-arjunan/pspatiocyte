...
legendTitles = ['Spatiocyte ($\Delta t=0.48\ \mathrm{ms}, T=139\ \mathrm{s}$)','pSpatiocyte (8 cores) ($\Delta t=0.51\ \mathrm{ms},\ T=47\ \mathrm{s}$)','Smoldyn ($\Delta t=1\ \mathrm{ms},\ T=449\ \mathrm{s}$)','Parallel ReaDDy (8 cores) ($\Delta t=1\ \mathrm{ms}, T=549\ \mathrm{s}$)','Serial ReaDDy ($\Delta t=1\ \mathrm{ms}, T=2197\ \mathrm{s}$)']
    ...
    for i in range(colSize):
      if (i == 0):
        plot(data[0], data[i+1]/volume, ls=lines[f], 
            color=tableau20[colors[f]], label=legendTitles[4],
